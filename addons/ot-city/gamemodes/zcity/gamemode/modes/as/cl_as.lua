...
local teams = {
	[0] = {
		objective = "Устроить серверу самый токсичный лобби-экспириенс и отправить как можно больше челов в спектры.",
		name = "Главный шизострел",
		color1 = Color(200, 40, 40),
		color2 = Color(200, 40, 40)
	},
	[1] = {
		objective = "Выжить, не словить фулл аим в лицо и по возможности не стать контентом для киллфида.",
		name = "Гражданский",
...
CreateEndMenu = function(winner)
	...
	hmcdEndMenu.PaintOver = function(self, w, h)
		...
		local title
		if winner == 0 then
			title = "Шизострел сделал минус лобби"
		elseif winner == 1 then
			title = "Люди каким-то чудом выжили"
		else
			title = "Раунд закончился, все в ахуе"
		end
	...
	for i, ply in player.Iterator() do
		...
		row.Paint = function(self, w, h)
			...
			local nameText = ply:GetPlayerName() or "Ливнул как легенда"
			...
			local status
			if not alive then
				status = " - словил вайб кладбища"
			elseif isWinner then
				status = " - выжил и теперь сигма"
			else
				status = ""
			end
...
function MODE:HUDPaint()
	...
	if shooterSpawnTime > CurTime() then
		...
		local text = "Шизострел залетит через: " .. string.FormattedTime(timeLeft, "%02i:%02i")
	...
	if timeBeforeSWAT > 0 and CurTime() >= shooterSpawnTime then
		...
		local text = "SWAT влетит как платная подписка через: " .. string.FormattedTime(timeBeforeSWAT, "%02i:%02i")
	...
	if showStartIntro then
		...
		if teamData then
			draw.SimpleText("ZBattle | Кибербуллер", "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0, 162, 255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

			local roleColor = teamData.color1
			roleColor.a = 255 * fade
			draw.SimpleText("Ты сегодня: " .. teamData.name, "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.5, roleColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
