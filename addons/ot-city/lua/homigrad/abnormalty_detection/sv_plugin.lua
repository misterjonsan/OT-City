...
local function abno_mdata_key(prefix, stat_name)
    return "abno_" .. tostring(prefix) .. "_" .. tostring(stat_name)
end

local function abno_defaults_apply(ply)
    if ply:GetMData(abno_mdata_key("stats", "punishment"), nil) == nil then
        ply:SetMData(abno_mdata_key("stats", "punishment"), 0)
    end
    if ply:GetMData(abno_mdata_key("stats", "rituals_amt"), nil) == nil then
        ply:SetMData(abno_mdata_key("stats", "rituals_amt"), 0)
    end
    if ply:GetMData(abno_mdata_key("consequences", "amt"), nil) == nil then
        ply:SetMData(abno_mdata_key("consequences", "amt"), 0)
    end
    if ply:GetMData(abno_mdata_key("knowledge", "json"), nil) == nil then
        ply:SetMData(abno_mdata_key("knowledge", "json"), util.TableToJSON({}))
    end
end

local function abno_wait_loaded(ply, cb)
    ...
    timer.Create("abno_wait_loaded_" .. sid, 0.2, 0, function()
        if not IsValid(ply) then
            timer.Remove("abno_wait_loaded_" .. sid)
            if cb then cb(false) end
            return
        end
        if mdata and mdata.IsLoaded and mdata:IsLoaded(ply) then
...
function PLUGIN.SetPlayerStat(ply, stat_name, value)
    ...
    PLUGIN.PlayerStats[sid] = PLUGIN.PlayerStats[sid] or {}
    PLUGIN.PlayerStatsPlayers[sid] = ply
    PLUGIN.PlayerStats[sid][stat_name] = value
    if ply.AbnormaltiesReady then
        ply:SetMData(abno_mdata_key("stats", stat_name), value)
    else
...
function PLUGIN.GetPlayerStat(ply, stat_name)
    ...
    if PLUGIN.PlayerStats[sid] and PLUGIN.PlayerStats[sid][stat_name] != nil then
        return PLUGIN.PlayerStats[sid][stat_name]
    end
    if ply.AbnormaltiesReady then
        local v = ply:GetMData(abno_mdata_key("stats", stat_name), nil)
        if v == nil then return nil end
        PLUGIN.PlayerStats[sid] = PLUGIN.PlayerStats[sid] or {}
        PLUGIN.PlayerStats[sid][stat_name] = v
        PLUGIN.PlayerStatsPlayers[sid] = ply
        return v
    end
    return nil
...
function PLUGIN.SetKnowledge(ply, knowledge)
    ...
    if ply.AbnormaltiesReady then
        ...
        ply:SetMData(abno_mdata_key("knowledge", "json"), util.TableToJSON(knowledge))
        return true
    else
...
function PLUGIN.GetConsequences(ply, callback)
    if not IsValid(ply) then return false end
    if not ply.AbnormaltiesReady then
        abno_wait_loaded(ply, function(ok)
            if not ok or not IsValid(ply) then return end
            abno_defaults_apply(ply)
            local consequences = tonumber(ply:GetMData(abno_mdata_key("consequences", "amt"), 0)) or 0
            local knowledge_json = ply:GetMData(abno_mdata_key("knowledge", "json"), util.TableToJSON({}))
            local knowledge = util.JSONToTable(knowledge_json) or {}
            local stats = {
                punishment = tonumber(ply:GetMData(abno_mdata_key("stats", "punishment"), 0)) or 0,
                rituals_amt = tonumber(ply:GetMData(abno_mdata_key("stats", "rituals_amt"), 0)) or 0
            }
            if callback then
                callback(ply, consequences, knowledge, stats, true)
            end
        end)
        return false
    end
    return ply:GetNWInt("AbnormaltiesConsequences", 0)
end

function PLUGIN.LoadConsequences(ply)
    if not IsValid(ply) then return end
    if ply.AbnormaltiesReady then return end
    abno_wait_loaded(ply, function(ok)
        if not ok or not IsValid(ply) then return end
        abno_defaults_apply(ply)
        local consequences = tonumber(ply:GetMData(abno_mdata_key("consequences", "amt"), 0)) or 0
        ...
        PLUGIN.PlayerStatsPlayers[steam_id_64] = ply
        for stat_name, _ in pairs(PLUGIN.PlayerStats[steam_id_64]) do
            if stats[stat_name] != nil then
                if PLUGIN.PlayerStatsToAddAfterLoad[stat_name] then
                    PLUGIN.PlayerStats[steam_id_64][stat_name] = (tonumber(stats[stat_name]) or 0) + (tonumber(PLUGIN.PlayerStats[steam_id_64][stat_name]) or 0)
                else
                    PLUGIN.PlayerStats[steam_id_64][stat_name] = tonumber(stats[stat_name]) or 0
                end
            end
        end
        if PLUGIN.PlayerStats[steam_id_64]["punishment"] == nil then
            PLUGIN.PlayerStats[steam_id_64]["punishment"] = stats.punishment
        end
        if PLUGIN.PlayerStats[steam_id_64]["rituals_amt"] == nil then
            PLUGIN.PlayerStats[steam_id_64]["rituals_amt"] = stats.rituals_amt
        end
        ply:SetMData(abno_mdata_key("stats", "punishment"), PLUGIN.PlayerStats[steam_id_64]["punishment"])
        ply:SetMData(abno_mdata_key("stats", "rituals_amt"), PLUGIN.PlayerStats[steam_id_64]["rituals_amt"])
    end)
end
...
function PLUGIN.SetConsequences(ply, amt, no_punishment)
    ...
    if not no_punishment then
        PLUGIN.SetPlayerStat(ply, "punishment", math.min(punishment + 360 * 2, 1000))
    end
    if ply.AbnormaltiesReady then
        ply:SetNWInt("AbnormaltiesConsequences", amt)
        ply:SetMData(abno_mdata_key("consequences", "amt"), math.Truncate(amt))
        local knowledge = ply.AbnormaltiesKnowledge or {}
        local knowledge_changed = false
        if not knowledge["consequences"] then
        ...
        if not knowledge["instabillity"] then
        ...
        if not knowledge["positive_instabillity"] then
            if amt >= 300 then
        ...
        if not knowledge["negative_instabillity"] then
            if amt <= -300 then
        ...
        if not knowledge["insanity"] then
...
hook.Add("PlayerDisconnected", "Abnormalties_SQLSave", function(ply)
    ...
    if sid and sid != "0" then
        PLUGIN.PlayerStats[sid] = nil
        PLUGIN.PlayerStatsPlayers[sid] = nil
    end
end)
    ...
    if ply.AbnormaltiesReady then
        ...
        if punishment_mul > 0 and ply:Alive() then
            ...
            if abs_consequences >= 50 then
                if consequences > 0 then
                    if bleeding_chance > 0 then
                        ...
                        if ply.AbnormaltiesConsequencesDamagePower >= 1 and (not ply.AbnormaltiesConsequencesDamagePowerCDTime or ply.AbnormaltiesConsequencesDamagePowerCDTime <= CurTime()) then
                    ...
                    if bleeding_chance > 0 then
                        ...
                        if ply.AbnormaltiesConsequencesBleedingPower >= 1 and (not ply.AbnormaltiesConsequencesBleedingPowerCDTime or ply.AbnormaltiesConsequencesBleedingPowerCDTime <= CurTime()) then
            ...
            if consequences >= 300 then
                if not ply.AbnormaltiesNextBenefitsTime or ply.AbnormaltiesNextBenefitsTime <= CurTime() then
...
function PLUGIN.FindPlyInZone(zone, caller, caller_priority, check_function)
    ...
    for _, ent in ipairs(ents.FindInSphere(zone.Pos, zone.Radius)) do
        if ent:IsPlayer() and ent:Alive() then
            if not check_function or check_function(ent) then
        ...
        if uselessness == 0 then
            ...
            local found_zone = PLUGIN.DoWithinZones(pos, function(zone_id, zone)
                ...
                if zone.Points > PLUGIN.HotZonePoints then
                    if not PLUGIN.HotZones[zone_id] then
...
hook.Add("Think", "Abnormalties", function()
    if not PLUGIN.NextZoneCheck or PLUGIN.NextZoneCheck <= CurTime() then
...
hook.Add("HG_PlayerSay", "Abnormalties", function(ply, txtTbl, text)
    if GetGlobalBool("AbnormaltiesEnabled", false) and IsValid(ply) then
        if ply:Alive() then
            if not ply.AbnormaltiesNextRequestStats or ply.AbnormaltiesNextRequestStats <= CurTime() then
	...
	ABNO Equalizers: ]] .. math.Round(ply.Abnormalties_Equalizers or 0) .. [[
            ...
            if string.StartsWith(text, cmd_find_phrase) then
                if not ply.AbnormaltiesNextFindPhrase or ply.AbnormaltiesNextFindPhrase <= CurTime() then
                    if ply.AbnormaltiesReady then
                        local knowledge = ply.AbnormaltiesKnowledge
...
hook.Add("HomigradDamage", "Abnormalties_Equalizers", function(ply, dmg, hitgroup, ent, harm)
    if not ply.AbnormaltiesDoNotCountEqualizers then
