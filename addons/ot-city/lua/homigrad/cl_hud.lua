...
if CLIENT then
	local function IsModeratorPly(ply)
		if not IsValid(ply) then return false end
		local grp = ply:GetUserGroup()
		return grp == "moderator" or grp == "operator" or grp == "sponsor"
	end
	...
	hook.Add( "PlayerBindPress", "PlayerBindPressExample2huy", function( ply, bind, pressed )
		if string.find(bind, "+menu") then
			...
			if (bind == "+menu") then
				...
				if lply:IsAdmin() or IsModeratorPly(lply) then return end
			end
...
local randomGestures = {
	...
	{"Окей", function() RunConsoleCommand("hg_hand_gesture", "okayhand") end},
	{"Флип", function() RunConsoleCommand("hg_hand_gesture", "flipoff") end},
	{"Палец вверх 2", function() RunConsoleCommand("hg_hand_gesture", "thumbsup") end},
	{"Визор", function() RunConsoleCommand("hg_hand_gesture", "visordown") end},
	{"Взрыв", function() RunConsoleCommand("hg_hand_gesture", "explosion") end},
	{"Дверь вперед", function() RunConsoleCommand("hg_hand_gesture", "door_open_forward") end},
	{"Дверь назад", function() RunConsoleCommand("hg_hand_gesture", "door_open_back") end},
	{"Дверь", function() RunConsoleCommand("hg_hand_gesture", "usedoor") end},
}

local randomGestureActs = {}
for _, gesture in ipairs(randomGestures) do
	if not istable(gesture) then
		randomGestureActs[#randomGestureActs + 1] = gesture
	end
end

local function randomGesture()
	if #randomGestureActs == 0 then return end
	RunConsoleCommand("act", randomGestureActs[math.random(#randomGestureActs)])
end
