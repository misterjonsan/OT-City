-- ведь мой кеш весь потрачен на кортеж
local TPIKBones = {
...
//
local hg_tpik_maxdist = ConVarExists("hg_tpik_maxdist") and GetConVar("hg_tpik_maxdist") or CreateClientConVar("hg_tpik_maxdist", 900, true, false, "", 0, 10000)
local hg_tpik_occlusion = ConVarExists("hg_tpik_occlusion") and GetConVar("hg_tpik_occlusion") or CreateClientConVar("hg_tpik_occlusion", 1, true, false, "", 0, 1)
local hg_tpik_vis_interval = ConVarExists("hg_tpik_vis_interval") and GetConVar("hg_tpik_vis_interval") or CreateClientConVar("hg_tpik_vis_interval", 0.12, true, false, "", 0.01, 1)

local hg_tpik_bonecache = setmetatable({}, {__mode = "k"})

function hg.GetCachedBoneIndex(ent, bone)
    if not IsValid(ent) then return end
    local mdl = ent:GetModel()
    local cache = hg_tpik_bonecache[ent]
    if not cache or cache.__mdl ~= mdl then
        cache = {__mdl = mdl}
        hg_tpik_bonecache[ent] = cache
    end
    local idx = cache[bone]
    if idx == nil then
...
local function hg_ResetWepIK(wpn)
    if not IsValid(wpn) then return end
    wpn.lhandik = false
    wpn.rhandik = false
end

function hg.ShouldRenderTPIK(ent, ply)
    local lply = LocalPlayer()
    if not IsValid(lply) then return false end
    if not IsValid(ent) or not IsValid(ply) then return false end

    if ply == lply or ent == lply or GetViewEntity() == ply or GetViewEntity() == ent then
        return true
    end

    if not lply:Alive() and lply:GetNWEntity("spect") == ply and lply:GetNWInt("viewmode", 0) == 1 then
        return true
    end

    if not ent:TestPVS(lply) then
        return false
    end

    local maxdist = hg_tpik_maxdist:GetFloat()
    if maxdist > 0 then
        local distpos = ent:WorldSpaceCenter()
        local head = hg.GetCachedBoneIndex(ent, "ValveBiped.Bip01_Head1")
        if head then
            local headmat = ent:GetBoneMatrix(head)
            if headmat then
                distpos = headmat:GetTranslation()
            end
        end
        if lply:EyePos():DistToSqr(distpos) > maxdist * maxdist then
            return false
        end
    end

    if not hg_tpik_occlusion:GetBool() then
        return true
    end

    local ct = CurTime()
    if ent._hg_tpik_vis_next and ent._hg_tpik_vis_next > ct then
        return ent._hg_tpik_vis_cached == true
    end

    ent._hg_tpik_vis_next = ct + hg_tpik_vis_interval:GetFloat()
    ...
    local tr = util.TraceLine({
        start = lply:EyePos(),
        endpos = target,
        filter = {lply, lply:GetVehicle(), ent, ply},
        mask = MASK_VISIBLE
    })

    ent._hg_tpik_vis_cached = not tr.Hit or tr.Entity == ent or tr.Entity == ply
    return ent._hg_tpik_vis_cached
end
//
//
...
function hg._DeprecatedDoTPIK(ply, ent, rhmat, lhmat)
    ...
    local shouldfullupdate = dt > 0.01

    if shouldfullupdate then
        ply.lastTPIK = SysTime()
    end

    local ply_spine_index = hg.GetCachedBoneIndex(ent, "ValveBiped.Bip01_Spine4")
    ...
    local ply_head_index = hg.GetCachedBoneIndex(ent, "ValveBiped.Bip01_Head1")
    if not ply_head_index then return end
    local ply_head_matrix = ent:GetBoneMatrix(ply_head_index)
    if not ply_head_matrix then return end
    ...
    local ply_l_upperarm_index = hg.GetCachedBoneIndex(ent, "ValveBiped.Bip01_L_UpperArm")
    local ply_r_upperarm_index = hg.GetCachedBoneIndex(ent, "ValveBiped.Bip01_R_UpperArm")
    local ply_l_forearm_index = hg.GetCachedBoneIndex(ent, "ValveBiped.Bip01_L_Forearm")
    local ply_r_forearm_index = hg.GetCachedBoneIndex(ent, "ValveBiped.Bip01_R_Forearm")
    local ply_l_hand_index = hg.GetCachedBoneIndex(ent, "ValveBiped.Bip01_L_Hand")
    local ply_r_hand_index = hg.GetCachedBoneIndex(ent, "ValveBiped.Bip01_R_Hand")

    if not ply_l_upperarm_index then return end
    if not ply_r_upperarm_index then return end
    if not ply_l_forearm_index then return end
    if not ply_r_forearm_index then return end
    if not ply_l_hand_index then return end
    if not ply_r_hand_index then return end
    ...
    if not rhik2 then
        local lpos = WorldToLocal(ply_r_hand_matrix:GetTranslation(), angle_zero, eyepos, eyeang)
        ply.last_rh_pos = lpos

        if ply.last_rh_pos2 then
            local pos = LocalToWorld(ply.last_rh_pos2, angle_zero, eyepos, eyeang)
        ...
        if ply.last_rh_pos then
            local pos = LocalToWorld(ply.last_rh_pos, angle_zero, eyepos, eyeang)
    ...
    local headmat = ent:GetBoneMatrix(ply_head_index)
    if not headmat then return end
    local angasd = headmat:GetAngles()
    angasd:RotateAroundAxis(angasd:Up(), -90)
    angasd:RotateAroundAxis(angasd:Forward(), -90)

    if shouldfullupdate or not ply.ply_r_upperarm_pos then
    ...
    if rhik then
        ...
        local wrst = hg.GetCachedBoneIndex(ent, "ValveBiped.Bip01_R_Wrist")
        if wrst then
        ...
        local ulna = hg.GetCachedBoneIndex(ent, "ValveBiped.Bip01_R_Ulna")
        if ulna then
            local wmat = ent:GetBoneMatrix(ulna)
            if wmat then
                wmat:SetAngles(ply_r_forearm_angle)
                ent:SetBoneMatrix(ulna, wmat)
            end
        end
    end
    ...
    if not lhik2 or ply.pullingTowards then
        local lpos = WorldToLocal(ply_l_hand_matrix:GetTranslation(), angle_zero, eyepos, eyeang)
        ply.last_lh_pos = lpos

        if ply.last_lh_pos2 then
            local pos = LocalToWorld(ply.last_lh_pos2, angle_zero, eyepos, eyeang)
            local lerp = ply.lerp_lh

            if ply.pullingTowards then
                if not IsValid(self) or not self.GetWM or not IsValid(self:GetWM()) or self ~= ply.pullingTowardsWeapon or (ply.pullingTowardsStart and ((ply.pullingTowardsStart + ply.pullingTowardsTime) < CurTime())) then
                    ...
                    local spine2 = hg.GetCachedBoneIndex(ent, "ValveBiped.Bip01_Spine2")
                    if spine2 then
                        local spine2mat = ent:GetBoneMatrix(spine2)
                        local pullbone = hg.GetCachedBoneIndex(ent, ply.pullingTowards)
                        local pullmat = pullbone and ent:GetBoneMatrix(pullbone) or nil
                        if spine2mat and pullmat then
                            local ang = spine2mat:GetAngles()
                            ang:RotateAroundAxis(ang:Right(), -90)
                            ang:RotateAroundAxis(ang:Up(), 90)
                            ang:RotateAroundAxis(ang:Right(), -30)

                            local ppos = pullmat:GetTranslation()
                            local pang = pullmat:GetAngles()

                            ply_l_hand_matrix:SetTranslation(LerpVector(lerp, ply_l_hand_matrix:GetTranslation(), ppos + pang:Right() * -4 + pang:Up() * 5))
                            ply_l_hand_matrix:SetAngles(LerpAngle(math.min(lerp * 2, 1), ply_l_hand_matrix:GetAngles(), ang))

                            if ((((ply.pullingTowardsStart + ply.pullingTowardsTime - CurTime()) / ply.pullingTowardsTime) * 2 - 1) < 0) then
                                if IsValid(ply.pullingTowardsModel) and ply.pullingTowardsOffsets then
                                    ...
                                    local ilerp = math.max(((((1 - (ply.pullingTowardsStart + ply.pullingTowardsTime - CurTime()) / ply.pullingTowardsTime)) - 0.5) * 2 - 0.6) / 0.4, 0)

                                    local wmbone = self:GetWM() and self:GetWM():GetBoneMatrix(ply.pullingTowardsOffsets[3]) or nil
                                    if wmbone then
                                        local pos1, ang1 = LocalToWorld(ply.pullingTowardsOffsets[4], ply.pullingTowardsOffsets[5], wmbone:GetTranslation(), wmbone:GetAngles())
                                        ang1:RotateAroundAxis(ang1:Up(), -90)

                                        local dpos = LerpVector(ilerp, pos2, pos1)
                                        local dang = LerpAngle(ilerp, ang2, ang1)

                                        ply.pullingTowardsModel:SetPos(dpos)
                                        ply.pullingTowardsModel:SetAngles(dang)
        ...
        if ply.last_lh_pos then
            local pos = LocalToWorld(ply.last_lh_pos, angle_zero, eyepos, eyeang)
            ply_l_hand_matrix:SetTranslation(LerpVector(ply.lerp_lh, pos, ply_l_hand_matrix:GetTranslation()))
        end
    end

    if IsValid(self) and IsValid(self.OwOmodel) and developer:GetBool() and LocalPlayer():IsSuperAdmin() and self.lmagpos3 then
    ...
    if shouldfullupdate or not ply.ply_l_upperarm_pos then
    ...
    if lhik then
        ...
        local wrst = hg.GetCachedBoneIndex(ent, "ValveBiped.Bip01_L_Wrist")
        if wrst then
            local wmat = ent:GetBoneMatrix(wrst)
            if wmat then
        ...
        local ulna = hg.GetCachedBoneIndex(ent, "ValveBiped.Bip01_L_Ulna")
        if ulna then
            local wmat = ent:GetBoneMatrix(ulna)
            if wmat then
                wmat:SetAngles(ply_l_forearm_angle)
...
function hg.MainTPIKFunction(ent, ply, wpn)
    ...
    if ent ~= ply and not hg.ShouldRenderTPIK(ent, ply) then
        hg_ResetWepIK(wpn)
        return
    end
    ...
    if should then
        ...
        if ply:InVehicle() then
            ...
            if IsValid(Car) then
                ...
                if bone and adjust and Car:GetBoneMatrix(bone) and wpn and not wpn.reload then
        ...
        if IsValid(wpn) and wpn:GetClass() == "weapon_hands_sh" and ply:GetNetVar("headcrab") then
            local head = hg.GetCachedBoneIndex(ent, "ValveBiped.Bip01_Head1")
            if head then
                local bone_matrix = ent:GetBoneMatrix(head)
                if bone_matrix then
    ...
    if ent ~= ply and ent.organism and ent.organism.stamina and ent.organism.stamina[1] then
        ...
        local rf11 = hg.GetCachedBoneIndex(ent, "ValveBiped.Bip01_R_Finger11")
        if rf11 and ent:GetManipulateBoneAngles(rf11)[2] < 0 then
            local rh = hg.GetCachedBoneIndex(ent, "ValveBiped.Bip01_R_Hand")
            if rh then
        ...
        local lf11 = hg.GetCachedBoneIndex(ent, "ValveBiped.Bip01_L_Finger11")
        if lf11 and ent:GetManipulateBoneAngles(lf11)[2] < 0 then
            local lh = hg.GetCachedBoneIndex(ent, "ValveBiped.Bip01_L_Hand")
            if lh then
