util.AddNetworkString("hg headshot sound")

hg.HeadshotSoundDelay = 0.05

local lastHeadshotSound = {}

local function headshotSoundTargetDead(org)
	if not org then return false end

	if org.dead or org.alive == false then return true end

	local owner = org.owner

	if not IsValid(owner) then return true end
	if owner:GetClass() == "prop_ragdoll" then return true end
	if owner:IsPlayer() and not owner:Alive() then return true end
	if owner.Health and owner:Health() <= 0 then return true end

	return false
end

function hg.HeadshotSound(dmgInfo, org)
	local attacker = dmgInfo:GetAttacker()

	if not IsValid(attacker) or not attacker:IsPlayer() then return end
	if not attacker:Alive() then return end
	if org and attacker == org.owner then return end
	if headshotSoundTargetDead(org) then return end

	local time = CurTime()

	if (lastHeadshotSound[attacker] or 0) > time then return end

	lastHeadshotSound[attacker] = time + hg.HeadshotSoundDelay

	net.Start("hg headshot sound", true)
	net.Send(attacker)
end

hook.Add("PlayerDisconnected", "homigrad-headshot-sound", function(ply)
	lastHeadshotSound[ply] = nil
end)
