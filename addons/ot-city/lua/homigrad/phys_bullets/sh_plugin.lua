...
PLUGIN.MaxKeyBits = 13
PLUGIN.MaxAmmoIDBits = 8
PLUGIN.MaxVelocityLenBits = 16
PLUGIN.MaxVelocityBits = 8
PLUGIN.MaxVelocityInt = 2^(PLUGIN.MaxVelocityBits - 1) - 1
PLUGIN.FirstPenetrationMul = 0.006
...
PLUGIN.NetCDUpdateBullet = PLUGIN.NetCDUpdateBullet or 0.05
...
local function translate_default_bullet_to_phys(bullet)
	...
	local hg_ammo_table = hg.ammotypeshuy and hg.ammotypeshuy[game.GetAmmoName(bullet.AmmoID)] or game.GetAmmoData(bullet.AmmoID)
...
PLUGIN.NetworkTableFull = {
	...
	{ "AirResistMul", net.WriteFloat, net.ReadFloat },
...
function PLUGIN.Class_Bullet:Think()
	...
	if hg.IsChanged and hg.IsChanged(self.Size, "Size", self) then
		local size = self.Size / 2
		self.SizeMins = Vector(-size, -size, -size)
		self.SizeMaxs = Vector(size, size, size)
	elseif not self.SizeMins or not self.SizeMaxs then
		local size = (self.Size or 0) / 2
		self.SizeMins = Vector(-size, -size, -size)
		self.SizeMaxs = Vector(size, size, size)
	end
	...
	while trace_hit and vel_vector do
		...
		if self.PenetratingMaterial then
			if not trace.AllSolid then
				...
				for penetrate_iteration = 1, max_penetrate_iterations do
					...
					if not penetration_trace.StartSolid and penetration_trace.Hit then
						penetration_pos = penetration_trace.HitPos
						break
					elseif not penetration_trace.AllSolid then
				...
				if penetration_pos then
					...
					if SERVER then
						...
						if not trace_backwards.StartSolid then
					...
					self.PenetratingStartPos = last_unsure_penetration_pos
		...
		if SERVER and IsValid(trace.Entity) then
			...
			if not self.AttackedEnts[trace.Entity] then
	...
	if SERVER and not self.NoNetworkUpdate then
		if not self.LastUpdateTime or (self.LastUpdateTime + PLUGIN.NetCDUpdateBullet <= CurTime()) then
			...
			if PLUGIN.NetworkBulletUpdate then
...
function PLUGIN.Class_Bullet:Hit(trace, len, len_before)
	...
	local new_vel_normal, len_left, ricochet, ang_diff, stopped = PLUGIN.CalcHit(trace, len, len_before, self.TraceMask, self.Penetration, self.SizeMins, self.SizeMaxs)
	len = len_left

	if ricochet then
		local len_subtract_frac = (90 - ang_diff) / 90
		local resist_mul = PLUGIN.CalcMaterialResist(self.PenetratingMaterial)
		len_before = len_before - math.min(resist_mul * self.AirResistMul * 140 * len_subtract_frac * len_before * len_before, len_before)
		len = math.min(len, len_before)
		...
		if trace.HitSky then
			...
			local resist = math.min(resist_mul * self.AirResistMul / self.PhysPenetrationMul * 200000000 / self.Mass, len_before)
...
function PLUGIN.Class_Bullet:Remove()
	...
	if SERVER then
		if self.CreationTime ~= CurTime() then
			if PLUGIN.NetworkBulletRemove then
...
function PLUGIN.CreateBullet(bullet)
	...
	bullet.AirResistMul = bullet.AirResistMul or 0.0001
	...
	if SERVER and not bullet.Removed and not bullet.NoNetwork then
		if PLUGIN.NetworkBulletFull then
...
function PLUGIN.CalcHit(trace, len, len_before, trace_mask, penetration, size_mins, size_maxs)
	...
	if len_before < 2500 then
		...
		if trace_new.HitTexture == "**studio**" then
			...
			for penetrate_iteration = 1, max_penetrate_iterations do
				...
				if not trace_penetrating.StartSolid then
					ricochet = false
					break
				else
			...
			if trace.HitSky then
				ricochet = false
			elseif not trace_new.AllSolid and trace.HitTexture ~= "**displacement**" then
				if not (size_mins and size_maxs) then
					ricochet = false
				end
			end
		end
	...
	if ricochet then
		...
		new_vel_normal = new_vel_normal + Vector(
			util.SharedRandom("Ricochet", -1, 1, CurTime() + 1) * 0.3,
			util.SharedRandom("Ricochet", -1, 1, CurTime() + 2) * 0.3,
			util.SharedRandom("Ricochet", -1, 1, CurTime() + 3) * 0.3
		) * ricochet_frac
...
PLUGIN:AddHook("Think", function()
	for key, bullet in pairs(PLUGIN.BulletsTable) do
		if bullet and bullet.Think then
			bullet:Think()
		end
	end
end)
...
if CLIENT then
	PLUGIN:AddHook("PostDrawTranslucentRenderables", function(bDepth, bSkybox)
		if not bSkybox then
			for key, bullet in pairs(PLUGIN.BulletsTable) do
				if bullet and bullet.Draw then
					bullet:Draw()
				end
			end
		end
...
if SERVER then
	local physbullets_replace_cvar = CreateConVar("physbullets_replace_default", "0", FCVAR_ARCHIVE, "", 0, 1)

	local function physbullets_apply_replace(value)
		local bool_value = tobool(value)
		SetGlobalBool("PhysBullets_ReplaceDefault", bool_value)
	end

	cvars.AddChangeCallback("physbullets_replace_default", function(_, _, new)
		physbullets_apply_replace(new)
	end, "PhysBullets_ReplaceDefault_Apply")

	timer.Simple(0, function()
		physbullets_apply_replace(physbullets_replace_cvar:GetBool())
	end)

	if sam and sam.command and sam.command.new then
		sam.command.new("physbullets_replace")
			:SetCategory("Server")
			:AddArg("bool", { optional = true, default = false })
			:OnExecute(function(ply, value)
				RunConsoleCommand("physbullets_replace_default", value and "1" or "0")
				if IsValid(ply) then
					ply:SetMData("physbullets_replace_last", value and 1 or 0)
					ply:SetMData("physbullets_replace_last_time", CurTime())
				end
			end)
		:End()
	else
		concommand.Add("physbullets_replace", function(ply, _, args)
			local value = tobool(tonumber(args[1] or "0") or args[1])
			RunConsoleCommand("physbullets_replace_default", value and "1" or "0")
			if IsValid(ply) then
				ply:SetMData("physbullets_replace_last", value and 1 or 0)
				ply:SetMData("physbullets_replace_last_time", CurTime())
...
hook.Add("EntityFireBullets", "PhysBullets_ReplaceDefault", function(ent, bullet)
	if GetGlobalBool("PhysBullets_ReplaceDefault", false) then
		if SERVER then
			if bullet.DontUsePhysBullets then
				return
			end

			if not IsValid(bullet.IgnoreEntity) then
