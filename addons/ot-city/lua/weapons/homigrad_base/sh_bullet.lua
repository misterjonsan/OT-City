...
function SWEP:GetTrace(bCacheTrace, desiredPos, desiredAng, NoTrace, closeanim)
	...
	if (not gunpos or not gunang) and IsValid(owner) and owner:IsPlayer() then
		local aimang = owner:EyeAngles()
		gunpos = gunpos or (owner:GetShootPos() - aimang:Forward() * 20)
		gunang = gunang or aimang
	end
...
function SWEP:FireBullet()
	...
	local lagcomp = SERVER and isply and hg.LagCompStart(owner)
