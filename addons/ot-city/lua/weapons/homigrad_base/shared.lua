...
if CLIENT then
	SWEP.HowToUseInstructions = "<font=ZCity_Tiny>"..string.upper( (input.LookupBinding("+use") or "ПРИВЯЖИТЕ КЛАВИШУ +use. ВВЕДИТЕ \"bind e +use\" В КОНСОЛИ, РАДИ ВСЕГО СВЯТОГО") ).." — чтобы поднять</font>"
end
SWEP.StartAtt = {}
...
hg.lagCompDepth = hg.lagCompDepth or {}
hg.lagCompTick = hg.lagCompTick or {}

local function lagCompEngineActive(ply)
	if not isfunction(ply.GetCurrentCommand) then return false end

	local ok, cmd = pcall(ply.GetCurrentCommand, ply)

	return ok and cmd ~= nil
end

function hg.LagCompStart(ply)
	if not SERVER then return false end
	if not IsValid(ply) or not ply:IsPlayer() then return false end
	if lagCompEngineActive(ply) then return false end

	local tick = engine.TickCount()
	local depth = hg.lagCompDepth[ply] or 0

	if depth > 0 and hg.lagCompTick[ply] ~= tick then
		ply:LagCompensation(false)
		depth = 0
	end

	if depth > 0 then
		hg.lagCompDepth[ply] = depth + 1

		return true
	end

	hg.lagCompTick[ply] = tick
	hg.lagCompDepth[ply] = 1
	ply:LagCompensation(true)

	return true
end

function hg.LagCompEnd(ply)
	if not SERVER then return end
	if not IsValid(ply) or not ply:IsPlayer() then return end

	local depth = hg.lagCompDepth[ply]

	if not depth or depth <= 0 then
		hg.lagCompDepth[ply] = nil
		hg.lagCompTick[ply] = nil
...
if SERVER then
	hook.Add("Think", "homigrad-weapons-lagcomp", function()
		if not next(hg.lagCompDepth) then return end

		local tick = engine.TickCount()

		for ply, depth in pairs(hg.lagCompDepth) do
			if not IsValid(ply) or not ply:IsPlayer() then
				hg.lagCompDepth[ply] = nil
				hg.lagCompTick[ply] = nil
			elseif (depth or 0) > 0 and hg.lagCompTick[ply] ~= tick then
				hg.lagCompDepth[ply] = nil
				hg.lagCompTick[ply] = nil
				ply:LagCompensation(false)
			end
...
hook.Add("PlayerDisconnected", "homigrad-weapons-lagcomp", function(ply)
	hg.lagCompDepth[ply] = nil
	hg.lagCompTick[ply] = nil
end)
...
local CantDoIt = {
	"Но... Ведь ради столького стоит жить!",
	"Я... Я не могу этого сделать...",
	"Должен быть другой путь. Это не он!",
	"Я... Не могу заставить себя это сделать.",
	"Что я вообще делаю? Я так не могу."
}
--qol lmao
...
function SWEP:Shoot(override)
	...
	local now = CurTime()
	local wait = math.max(primary.Wait or 0.1, 0.01)
	local tick = engine.TickInterval()
	local ownr = self:GetOwner()
	local isply = IsValid(ownr) and !ownr:IsNPC()
	local last = self.NextShot or 0

	if !override and isply and last > now then return false end
	if !override and isply and (primary.NextFire or 0) > now then return false end

	if last <= 0 or (now - last) > wait then
		self.NextShot = now + wait
	else
		self.NextShot = last + wait
	end

	primary.Next = self.NextShot
	self:SetNextPrimaryFire(math.max(now, self.NextShot - tick))
	...
	if !override and isply and primary.RealAutomatic then
		local extra = 0
		while self.NextShot <= now and extra < 3 and self:Clip1() > 0 do
			self.NextShot = self.NextShot + wait
			extra = extra + 1
			self:PrimaryShoot()
			self:PrimaryShootPost()
		end
	...
	if owner:GetNWFloat("InLegKick",0) <= CurTime() and (!(IsValid(owner.FakeRagdoll) or IsValid(owner.FakeRagdollOld)) or false--[[self:Clip1() <= 0]]) and self:KeyDown(IN_ATTACK) and self:KeyDown(IN_USE) and ((self:GetButtstockAttack() + 1 * ((math.max(0, (self.weight - 3)) * 0.2) + 1) * (math.Clamp((180 - stam) / 90, 1, 2))) < CurTime()) and owner:GetVelocity():LengthSqr() < 250 * 250 and (SERVER or IsFirstTimePredicted()) then
		...
		if SERVER then
			//timer.Simple(0.15, function()
				local lagcomp = hg.LagCompStart(owner)
				...
				if lagcomp then hg.LagCompEnd(owner) lagcomp = nil end
			//end)
		end
	end
