	...
	if IsValid(phys) then
		...
		if self.CarryEnt:GetClass() == "prop_ragdoll" then
			...
			if ply:KeyPressed(IN_RELOAD) then
				if not (ply2.noHead and (not hg.CPR or hg.CPR.Headless(ply2))) and ply2.organism then
			...
			if ply:KeyDown(IN_ATTACK) and !ply.organism.superfighter and !(org and ply.PlayerClassName == "furry" and org.owner.PlayerClassName != "furry") and !ply:IsBerserk() then
				...
				if org and bone == "ValveBiped.Bip01_Spine2" and trace.Hit then
					if self.firstTimePrint then
						if not (ply2.noHead and (not hg.CPR or hg.CPR.Headless(ply2))) then
							ply:ChatPrint("Вы начинаете проводить сердечно-лёгочную реанимацию (СЛР).")

							if not org.alive then
								ply:ChatPrint("Сердце остановилось. Шанс есть только в первую минуту.")
							end
						else
					...
					if (self.CPRThink or 0) < CurTime() then
						...
						if not org.alive and hg.CPR and isfunction(hg.CPR.Compress) then
							hg.CPR.Compress(ply, ply2, org)
						end
