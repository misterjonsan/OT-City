	...
	if self:GetIsOn() then
		tbl[#tbl + 1] = {function()
			...
			for i = 1, #self.Frequencies do
				...
				tbl1[#tbl1 + 1] = { function() RunConsoleCommand("hg_walkietalkie_adjust", station - self:GetHudFrequency() ) end, "Частота " .. station .. "MHz" }
				hg.CreateRadialMenu(tbl1)
			end
			return -1
		end, "Публичные частоты"}
		self:MenuAddAdjuster("Сменить  010.0 MHz", tbl, 010.0)
		self:MenuAddAdjuster("Сменить  001.0 MHz", tbl, 001.0)
		self:MenuAddAdjuster("Сменить  000.1 MHz", tbl, 000.1)
	end

	tbl[#tbl + 1] = {function()
		RunConsoleCommand("+reload")
		timer.Simple(0,function() RunConsoleCommand("-reload") end)
	end, self:GetIsOn() and "Отключить Рацию" or "Включить Рацию"}
