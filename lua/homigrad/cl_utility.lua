...
--\\ CL Utils setting adjustments
	if CLIENT then
		local decalsCvar = GetConVar("r_decals")
		if decalsCvar then
			local decalsFlags = decalsCvar:GetFlags()
			local decalsBlocked = bit.bor(FCVAR_REPLICATED or 8192, FCVAR_PROTECTED or 32, FCVAR_SERVER_CAN_EXECUTE or 65536, FCVAR_GAMEDLL or 4)
			if bit.band(decalsFlags, decalsBlocked) == 0 and decalsCvar:GetString() ~= "4096" then
				pcall(decalsCvar.SetString, decalsCvar, "4096")
			end
		end
