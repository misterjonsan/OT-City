...
hg.organism.lethal = hg.organism.lethal or {}

local lethal = hg.organism.lethal

lethal.BloodDead = 900
lethal.BloodCritical = 1900
lethal.HeartFatal = 0.85
lethal.LungFatal = 0.8
lethal.TorsoFatal = 4
lethal.BleedFatal = 20
lethal.OrganFatal = 3
lethal.Grace = 5
lethal.Drain = 14
lethal.CreditTime = 20

local function num(v)
	return isnumber(v) and v or 0
end

local function lungDamage(org)
	local r = istable(org.lungsR) and num(org.lungsR[1]) or 0
	local l = istable(org.lungsL) and num(org.lungsL[1]) or 0

	return r, l
end

function lethal.Torso(org)
	local r, l = lungDamage(org)

	return num(org.chest) + num(org.pelvis) + num(org.spine1) + num(org.spine2) + num(org.spine3) + num(org.stomach) + num(org.intestines) + num(org.liver) + num(org.heart) + r + l
end

function lethal.Organs(org)
	local r, l = lungDamage(org)
	local count = 0

	for _, value in ipairs({num(org.heart), num(org.liver), num(org.stomach), num(org.intestines), r, l}) do
		if value >= 0.9 then count = count + 1 end
	end

	return count
end

function lethal.Fatal(org)
	local r, l = lungDamage(org)

	if num(org.blood) <= lethal.BloodDead then return true end
	if num(org.heart) >= lethal.HeartFatal then return true end
	if r >= lethal.LungFatal and l >= lethal.LungFatal then return true end
	if num(org.internalBleed) >= lethal.BleedFatal then return true end
	if lethal.Organs(org) >= lethal.OrganFatal then return true end
	if lethal.Torso(org) >= lethal.TorsoFatal then return true end

	return false
end

function lethal.Dying(org)
	return num(org.dying) > 0
end

function lethal.Kill(owner, org)
	org.heartstop = true
	org.lungsfunction = false
	org.alive = false

	local attacker = org.lastAttacker

	if IsValid(attacker) and attacker ~= owner and num(org.lastAttackTime) + lethal.CreditTime > CurTime() then
		local dmg = DamageInfo()
		dmg:SetDamage(owner:Health() + 25)
		dmg:SetDamageType(DMG_BULLET)
		dmg:SetAttacker(attacker)
		dmg:SetInflictor(IsValid(org.lastInflictor) and org.lastInflictor or attacker)
...
function lethal.Think(owner, org, timeValue)
	if not IsValid(owner) or not owner:IsPlayer() then return end
	if not owner:Alive() or not org.alive then return end
	if owner.IsBerserk and owner:IsBerserk() then org.dying = 0 return end

	timeValue = num(timeValue)

	if timeValue <= 0 then return end

	local blood = num(org.blood)
	local rate = 0

	if lethal.Fatal(org) then
		org.dying = num(org.dying) + timeValue
		rate = lethal.Drain * (1 + num(org.dying) / lethal.Grace)
	elseif blood < lethal.BloodCritical then
		org.dying = 0
		rate = lethal.Drain * 0.5 * math.Clamp((lethal.BloodCritical - blood) / math.max(1, lethal.BloodCritical - lethal.BloodDead), 0, 1)
	else
		org.dying = 0
	end
	...
	local hp = owner:Health() - rate * timeValue

	if hp <= 0 then
		lethal.Kill(owner, org)
...
hook.Add("EntityTakeDamage", "hg-lethality-credit", function(ent, dmgInfo)
	if not IsValid(ent) then return end

	local org = ent.organism or (hg.RagdollOwner and IsValid(hg.RagdollOwner(ent)) and hg.RagdollOwner(ent).organism) or nil
	...
	if attacker == org.owner then return end

	org.lastAttacker = attacker
	org.lastInflictor = dmgInfo:GetInflictor()
	org.lastAttackTime = CurTime()
end)
...
hook.Add("Org Clear", "Main", function(org)
	...
	org.health = 100
	org.dying = 0
	org.lastAttacker = nil
	org.lastInflictor = nil
	org.lastAttackTime = 0
	org.canmove = true
	org.recoilmul = 1
	org.legstrength = 1
	org.meleespeed = 1
...
hook.Add("Org Think", "Main", function(owner, org, timeValue)
	...
	if owner:IsPlayer() and (org.healthRegen or 0) < CurTime() and not lethal.Dying(org) and (org.blood or 5000) > lethal.BloodCritical then
...
hook.Add("Org Think", "hg-lethality", function(owner, org, timeValue)
	lethal.Think(owner, org, timeValue)
end)
