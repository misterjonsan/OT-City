...
local pozivnie = {
    ...
    "Фокстрот","Танго","Сьерра","Юниформ","Кило","Янки","Регент",
    "Зулу","Омега","Нова","Вайпер","Гоуст","Рейвен",
}

local komand_pozivnie = {"Лидер", "Каратель", "Оверлорд", "Директор", "Контроль"}
...
local function GetSquadForCombine(ply)
    for name, data in pairs(CombineSquads.squads) do
        for i = #data.members, 1, -1 do
            ...
            if not IsValid(m) or m.PlayerClassName ~= "Combine" then
    ...
    local available = {}
    for _, cs in ipairs(pozivnie) do
        if not CombineSquads.usedCallsigns[cs] then
            table.insert(available, cs)
        end
    end

    if #available == 0 then
        return pozivnie[math.random(#pozivnie)]
    end
...
local function AssignCombineCallsign(ply, isLeader)
    ...
    if isLeader then
        return komand_pozivnie[math.random(#komand_pozivnie)] .. "-1"
    end

    local squadName = GetSquadForCombine(ply)
    local squad = CombineSquads.squads[squadName]
    if not squad then
        return pozivnie[math.random(#pozivnie)] .. "-" .. math.random(10, 99)
    end

    local num
    repeat
        num = math.random(10, 99)
    until not squad.usedNumbers[num]
...
local combine_subclasses = {
    novobranec = {
        ...
        loadout = {
            {weapon = "weapon_melee"},
            {weapon = "weapon_hg_hl2nade_tpik", count = 1},
            {weapon = "weapon_glock17", ammo_mult = 3},
            {weapon_random_pool = {"weapon_mp7", "weapon_mp7", "weapon_smg1"}, ammo_mult = 3}
        }
    },
    ...
    soldat = {
        ...
        loadout = {
            {weapon = "weapon_melee"},
            {weapon = "weapon_hg_hl2nade_tpik", count = 1},
            {weapon = "weapon_hk_usp", ammo_mult = 3},
            {weapon_random_pool = {"weapon_mp7", "weapon_mp7", "weapon_ar2", "weapon_osipr"}, ammo_mult = 3}
        }
    },
    ...
    shturmovik = {
        ...
        mat = {
            ...
            ["models/combine_soldier/combinesoldiersheet_player"] = "models/combine_soldier/combinesoldiersheet_shotgun"
        },
        loadout = {
            {weapon = "weapon_melee"},
            {weapon = "weapon_hg_flashbang_tpik", count = 1},
            {weapon = "weapon_hk_usp", ammo_mult = 3},
            {weapon = "weapon_breachcharge", count = 1},
            {weapon_random_pool = {"weapon_spas12", "weapon_m3super90"}, ammo_mult = 3}
        }
    },
    ...
    snajper = {
        ...
        loadout = {
            ...
            {weapon = "weapon_hg_hl2nade_tpik", count = 1},
            {weapon = "weapon_hk_usp", ammo_mult = 3},
            {weapon = "weapon_combinesniper", ammo_mult = 3}
        }
    },
    ...
    elita = {
        ...
        loadout = {
            {weapon = "weapon_melee"},
            {weapon = "weapon_hg_hl2nade_tpik", count = 2},
            {weapon = "weapon_hk_usp", ammo_mult = 3},
            {weapon = "weapon_osipr", ammo_mult = 3, extra_balls = 3}
        }
    },
    ...
    komandir = {
        ...
        mat = {
            ["models/combine_soldier/combine_elite_player"] = "models/combine_soldier/combine_elite",
            ["models/combine_soldier/combine_elite_player_head"] = "models/combine_soldier/combine_elite",
            ["models/combine_soldier/combinesoldiersheet_player"] = "models/combine_soldier/combine_prisonguard"
        },
        loadout = {
            {weapon = "weapon_melee"},
            {weapon = "weapon_hg_hl2nade_tpik", count = 2},
            {weapon = "weapon_hk_usp", ammo_mult = 3},
            {weapon_random_pool = {"weapon_osipr", "weapon_ar2", "weapon_mp7"}, ammo_mult = 4}
        }
    }
}

local subclass_order = {"novobranec", "soldat", "shturmovik", "snajper", "elita", "komandir"}

local function PickCombineSubclass()
    local total = 0
    for _, name in ipairs(subclass_order) do
        total = total + ((combine_subclasses[name] and combine_subclasses[name].chance) or 0)
    end
    ...
    for _, name in ipairs(subclass_order) do
        accum = accum + ((combine_subclasses[name] and combine_subclasses[name].chance) or 0)
        if roll <= accum then
            return name
        end
    end
...
local function giveSubClassLoadout(ply, subclass)
    local config = combine_subclasses[subclass] or combine_subclasses["soldat"]
    ...
    for _, item in ipairs(config.loadout or {}) do
        if item.weapon_random_pool then
            ...
            if IsValid(wep) and item.ammo_mult and wep.GetMaxClip1 then
...
function CLASS.On(self, data)
    ...
    local sub = self.subClass or PickCombineSubclass()
    local cfg = combine_subclasses[sub] or combine_subclasses["soldat"]
    local useModel = istable(cfg.models) and cfg.models[math.random(#cfg.models)] or cfg.models

    if useModel and useModel ~= "" then
    ...
    if cfg.mat then
        for k, v in pairs(cfg.mat) do
            local id = self:GetSubMaterialIdByName(k)
            if id and id >= 0 then
                self:SetSubMaterial(id, v)
            end
        end
    end
    ...
    local noEq = istable(data) and data.bNoEquipment
    if not noEq then
    ...
    local isLeader = (cfg.role == "Командир") or self.leader
    local callsign = AssignCombineCallsign(self, isLeader)

    self.oldname_cmb = self:GetNWString("PlayerName")
    if zb and zb.GiveRole then
        zb.GiveRole(self, cfg.role or "Солдат", Color(89,230,255))
    end

    self:SetNWString("PlayerRole", cfg.role or "Солдат")
...
if CLIENT then
    ...
    local function drawBGPanel(pos_x,pos_y)
    ...
    function CLASS.HUDPaint(self)
        ...
        local is_red = (role == "Командир" or role == "Штурмовик" or role == "Элита")
        ...
        do
            ...
            local col_bg = Color(bg_color.r,bg_color.g,bg_color.b,225)
            ...
            draw.DrawText("% | КРОВЬ","CMBFontSmall",
        ...
        do
            ...
            draw.DrawText("| ПУЛЬС","CMBFontSmall",
        ...
        do
            ...
            draw.DrawText("| ВЫНОСЛИВОСТЬ","CMBFontSmall",
                pos[1]+size[1]*0.065 + 12,
                pos[2]+(size[2]/2),
                is_red and color_hp2 or color_hp,
                TEXT_ALIGN_LEFT
        ...
        do
            ...
            local col_bg = Color(bg_color.r,bg_color.g,bg_color.b,225*silentlerp)
            silentclr.a = 225*silentlerp
            local txt = "ТИХИЙ РЕЖИМ"
        ...
        if IsValid(wep) and wep.Clip1 then
            local ammolerp = Lerp(frt,0,(wep:Clip1() < 0) and 0 or 1)
            local pos, size = drawBGPanel(0.93,0.98)
            surface.SetFont("CMBFontDefault")
            local _,txt_size_y = surface.GetTextSize(self:Armor())
            local col_bg = Color(bg_color.r,bg_color.g,bg_color.b,225*ammolerp)
            ...
            draw.DrawText("БОЕЗАПАС","CMBFontSmall",
                pos[1]+size[1]*0.085,
                pos[2]+(size[2]/1.8),
    ...
    hook.Add("RenderScreenspaceEffects","Combine_helmet",function()
        ...
        if lply:Alive() and lply.PlayerClassName == "Combine" then
            ...
            if role == "Элита" or role == "Штурмовик" or role == "Командир" then
...
if CLIENT then
    ...
    local function togglePNV()
        ...
        if pnv_enabled then
            ...
            hook.Add("RenderScreenspaceEffects","PNV_ColorCorrection",function()
                ...
                local cc = (class == "Элита" or class == "Штурмовик" or class == "Командир") and table.Copy(pnv_color_2) or table.Copy(pnv_color_1)
    ...
    hook.Add("RenderScreenspaceEffects","PNV_ColorCorrection",function()
        ...
        if pnv_enabled then
            local class = ply:GetNWString("PlayerRole")
            local cc = (class == "Элита" or class == "Штурмовик" or class == "Командир") and pnv_color_2 or pnv_color_1
            DrawColorModify(cc)
