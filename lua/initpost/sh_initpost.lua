...
local function estimate(file)
    ...
    local function try_parse_frame()
        ...
        if has_sync and valid_ver and valid_layer then
            ...
            if valid_bitrate and valid_sample then
                ...
                if not bitrate then
                    print("[X-CITY] Неизвестный битрейт (v", version, " l", layer, " idx", rshift(second_half, 12), ")")
                end
    ...
    while file:remaining() >= 4 do
        try_parse()
        l = l + 1
        if l > (5 * 1e6) then print("[X-CITY] Прерван разбор слишком большого MP3 (", framesParsed, " кадров)") break end
    end
...
local function estimate_data(data)
    if not isstring(data) then return 0 end
...
if CLIENT then
    ...
    if dxlevel < 90 then
        local function dxWarning()
            if SERVER then return end
            surface.SetDrawColor(5, 9, 16, 255)
            surface.DrawRect(0, 0, ScrW(), ScrH())

            surface.SetTextPos(ScrW() / 4, ScrH() / 2)
            surface.SetTextColor(31, 182, 255, 255)
            surface.SetFont("DermaLarge")
            surface.DrawText("Включи DirectX 9 или выше и перезапусти игру — иначе не поиграть.")
        end

        hook.Add("HUDPaint", "XC_DXWarning", dxWarning)
        hook.Add("HUDPaintBackground", "XC_DXWarning", dxWarning)
        hook.Add("PreDrawHUD", "XC_DXWarning", dxWarning)
        hook.Add("PostDrawHUD", "XC_DXWarning", dxWarning)
    end
end
...
if CLIENT then
    ...
    hook.Add("OnEntityCreated", "doorInstructions", function(ent)
        if SDOIsDoor(ent) then
            if CLIENT then
                local use = input.LookupBinding("+use") or "НЕ НАЗНАЧЕНО — ПРОПИШИ В КОНСОЛИ: bind e +use"
                local walk = input.LookupBinding("+walk") or "НЕ НАЗНАЧЕНО — ПРОПИШИ В КОНСОЛИ: bind alt +walk"
                local speed = input.LookupBinding("+speed") or "НЕ НАЗНАЧЕНО — ПРОПИШИ В КОНСОЛИ: bind shift +speed"
                ...
                ent.AdditionalInfoFunc = function()
                    return lply:KeyDown(IN_WALK) and "Открыть дверь для" .. math.Round(100 - math.min(math.abs(lply:EyeAngles().p) / 60, 1) * 100) .. "%" or ""
                end
            end
        end
    end)
