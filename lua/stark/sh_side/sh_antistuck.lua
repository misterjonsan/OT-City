...
util.AddNetworkString("DevLuaReloadMessage")

hook.Add("Initialize","DevLuaReloadNotify",function()

    timer.Simple(1,function()

        local time = os.date("%H:%M:%S")
        local reloader = "SERVER"

        for _,ply in player.Iterator() do
            if ply:IsSuperAdmin() and ply:IsListenServerHost() then
                reloader = ply:Nick()
                break
            end
        end

        net.Start("DevLuaReloadMessage")
        net.WriteString(reloader)
        net.WriteString(time)
        net.Broadcast()

        print("[DEV] Lua reload detected | "..time)
...
net.Receive("DevLuaReloadMessage",function()

    local who = net.ReadString()
    local time = net.ReadString()

    chat.AddText(
        Color(120,220,255),"[DEV-BLOG] ",
        Color(255,255,255),"Модули были ",
        Color(0,255,140),"перезагружены",
        Color(255,255,255)," | ",
        Color(255,200,0),time,
        Color(255,255,255)," | инициатор: ",
        Color(255,120,120),who
    )
