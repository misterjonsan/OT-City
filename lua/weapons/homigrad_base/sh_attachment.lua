...
if CLIENT then
	...
	local function refreshtbl()
	    ...
	    -- гарантируем валидного игрока
	    local ply = lply
	    if not IsValid(ply) then ply = LocalPlayer() end
	    if not IsValid(ply) then return tblcpy end

	    -- гарантируем таблицы инвентаря и аттачей
	    local inv = ply:GetNetVar("Inventory") or {}
	    local tbl = inv["Attachments"] or inv.Attachments or {}  -- <= если nil, станет {}

	    local wep = ply:GetActiveWeapon()
	    local achtbl = {}
	    if IsValid(wep) and ishgweapon(wep) then
	        achtbl = wep:GetNetVar("attachments") or {}
	    end

	    -- теперь pairs всегда по таблице
	    for _, att in pairs(tbl) do
	        if not att then continue end
	        table.insert(tblcpy, {att, false})
	    end

	    for _, att in pairs(achtbl) do
